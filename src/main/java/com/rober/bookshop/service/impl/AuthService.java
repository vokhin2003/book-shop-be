...
public class AuthService implements IAuthService {
    ...
    public void resendVerification(String email, String clientPlatform) {
        User user = this.userRepository.findByEmail(email)
                .orElseThrow(() -> new IdInvalidException("User not found with email"));
        if (user.isActive()) {
    ...
    public LoginResponseDTO handleUserLogin(LoginRequestDTO reqLoginDTO) {
        ...
        User savedUser = this.userRepository.findByEmail(reqLoginDTO.getUsername()).orElse(null);
        if (savedUser != null) {
    ...
    public LoginResponseDTO.UserGetAccount fetchAccount() {
        ...
        User savedUser = this.userRepository.findByEmail(email).orElse(null);
    ...
    public void createPassword(CreatePasswordRequestDTO request) {
        ...
        User user = userRepository.findByEmail(SecurityUtil.getCurrentUserLogin().orElseThrow(() -> new IdInvalidException("User not found in SecurityContext"))).orElseThrow(() -> new IdInvalidException("User not found in database"));
    ...
    public LoginResponseDTO outboundAuthenticate(String code) {
        ...
        LoginResponseDTO res = new LoginResponseDTO();
        Role userRole = roleRepository.findByName("CUSTOMER").orElseThrow(() -> new IdInvalidException("Role CUSTOMER not found"));
    ...
    public String handleResetRedirect(String token, String clientPlatform) {
        ...
        if (t == null || t.isRevoked() || t.getType() != TokenType.RESET_PASSWORD || Instant.now().isAfter(t.getExpiresAt())) {
            return isWeb ?
                    ...
                    :
                    "http://bromel.free.nf/forgot-return.html?status=error";
        }
        return isWeb ?
